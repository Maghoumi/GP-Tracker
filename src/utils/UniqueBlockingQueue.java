...
/**
 * A shameless copy of Java's ArrayBlockingQueue with only a minor difference:
 * the elements in this queue are unique. When a new element is added to the queue
 * the contains() method is called and the new element is only added to the queue
 * if it does not already exist in this queue. Another subtle difference of this
 * class is the availability of a blocking peek() method: you can "peek", get an
 * element without removing it, however, if the queue is empty, the calling thread
 * is blocked until an element is added to the queue.
 * 
 * @author Mehran Maghoumi
 * 
 */

public class UniqueBlockingQueue<E> extends AbstractQueue<E>
	...
	private void insert(E x) {
		// if we already have the element in the queue, just mimic an add!
		if (contains(x)) {
			notEmpty.signal();
			return;
		}
	 ...
	 *             if {@code capacity < 1}
	 */
	public UniqueBlockingQueue(int capacity, boolean fair) {
		if (capacity <= 0)
     ...
     * Will block the calling thread if the queue is empty!!
     *
     * @return the head of this queue
     */
	public E blockPeek() throws InterruptedException {
		final ReentrantLock lock = this.lock;
		lock.lockInterruptibly();
		try {
			while (count == 0)
	...
	private class Itr implements Iterator<E> {
		...
		Itr() {
			final ReentrantLock lock = UniqueBlockingQueue.this.lock;
			lock.lock();
			try {
				lastRet = -1;
