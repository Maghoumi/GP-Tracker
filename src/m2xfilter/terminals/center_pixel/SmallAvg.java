package m2xfilter.terminals.center_pixel;
...
public class SmallAvg extends CudaNode
{
	@Override
			...
			final Problem problem)
	{
		...
		Float4 feature = rd.instance.smallAvg;
		...
		switch((int)result) {
			...
			default:
				state.output.fatal("SAVGValue was " + ((int) result) + " Original: " + result + " Constraints not met :(");
		}
	}
	...
	public String getCudaAction() {
		return 	"float top; pop(top);" +
				"if (top == 0.0) push(smallAvg[tid].y/255.0);" +
				"else if (top == 1.0) push (smallAvg[tid].z/255.0);" +
				"else if (top == 2.0) push (smallAvg[tid].w/255.0);"+
				"else if (top == 3.0) {float4 value = smallAvg[tid];" +
