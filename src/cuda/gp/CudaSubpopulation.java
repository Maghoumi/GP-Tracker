...
public class CudaSubpopulation extends Subpopulation {
	...
	public void populate(EvolutionState state, int thread) {
		...
		// Should we seed the population?
		if (cuState.shouldSeed()) {
			Individual ind = cuState.getPopulationSeed();
			if (ind != null)	// The individual may be null at first
				individuals[start++] = cuState.getPopulationSeed();
		}
